...
<%
	...
	String fileName = multi.getFilesystemName(fname);
		...
		if (rs.next()) {
			if (fileName != null) {
				sql = "UPDATE donation SET d_donationId=?, d_donationName=?, d_donationPrice=?, d_goalPrice=?, d_donationPlace=?, d_donationPeriod=?, d_donationDescription=?, d_fileName=? WHERE d_donationId=?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, donationId);
				...
				pstmt.setString(9, donationId);
				pstmt.executeUpdate();
			} else {
				sql = "UPDATE donation SET  d_donationId=?, d_donationName=?, d_donationPrice=?, d_goalPrice=?, d_donationPlace=?, d_donationPeriod=?, d_donationDescription=? WHERE d_donationId=?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, donationId);
				pstmt.setString(2, donationName);
				pstmt.setInt(3, price);
